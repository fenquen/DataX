...
public class ConfigurationChecker {
    ...
    private static void paramCheck(Configuration config) {
        ...
        if (StringUtils.isBlank(proxy)) {
            throw DataXException.build(OcsWriterErrorCode.REQUIRED_VALUE, String.format("ocs服务地址%s不能设置为空", Key.PROXY));
        }
        String user = config.getString(Key.USER);
        if (StringUtils.isBlank(user)) {
            throw DataXException.build(OcsWriterErrorCode.REQUIRED_VALUE, String.format("访问ocs的用户%s不能设置为空", Key.USER));
        }
        String password = config.getString(Key.PASSWORD);
        if (StringUtils.isBlank(password)) {
            throw DataXException.build(OcsWriterErrorCode.REQUIRED_VALUE, String.format("访问ocs的用户%s不能设置为空", Key.PASSWORD));
        }

        String port = config.getString(Key.PORT, "11211");
        if (StringUtils.isBlank(port)) {
            throw DataXException.build(OcsWriterErrorCode.REQUIRED_VALUE, String.format("ocs端口%s不能设置为空", Key.PORT));
        }

        String indexes = config.getString(Key.INDEXES, "0");
        if (StringUtils.isBlank(indexes)) {
            throw DataXException.build(OcsWriterErrorCode.REQUIRED_VALUE, String.format("当做key的列编号%s不能为空", Key.INDEXES));
        }
        for (String index : indexes.split(",")) {
            try {
                if (Integer.parseInt(index) < 0) {
                    throw DataXException.build(OcsWriterErrorCode.ILLEGAL_PARAM_VALUE, String.format("列编号%s必须为逗号分隔的非负整数", Key.INDEXES));
                }
        ...
        if (StringUtils.isBlank(writerMode)) {
            throw DataXException.build(OcsWriterErrorCode.REQUIRED_VALUE, String.format("操作方式%s不能为空", Key.WRITE_MODE));
        }
        if (!EnumUtils.isValidEnum(WRITE_MODE.class, writerMode.toLowerCase())) {
            throw DataXException.build(OcsWriterErrorCode.ILLEGAL_PARAM_VALUE, String.format("不支持操作方式%s，仅支持%s", writerMode, StringUtils.join(WRITE_MODE.values(), ",")));
        }

        String writerFormat = config.getString(Key.WRITE_FORMAT, "text");
        if (StringUtils.isBlank(writerFormat)) {
            throw DataXException.build(OcsWriterErrorCode.REQUIRED_VALUE, String.format("写入格式%s不能为空", Key.WRITE_FORMAT));
        }
        if (!EnumUtils.isValidEnum(WRITE_FORMAT.class, writerFormat.toLowerCase())) {
            throw DataXException.build(OcsWriterErrorCode.ILLEGAL_PARAM_VALUE, String.format("不支持写入格式%s，仅支持%s", writerFormat, StringUtils.join(WRITE_FORMAT.values(), ",")));
        }

        int expireTime = config.getInt(Key.EXPIRE_TIME, 0);
        if (expireTime < 0) {
            throw DataXException.build(OcsWriterErrorCode.ILLEGAL_PARAM_VALUE, String.format("数据过期时间设置%s不能小于0", Key.EXPIRE_TIME));
        }

        int batchSiz = config.getInt(Key.BATCH_SIZE, 100);
        if (batchSiz <= 0) {
            throw DataXException.build(OcsWriterErrorCode.ILLEGAL_PARAM_VALUE, String.format("批量写入大小设置%s必须大于0", Key.BATCH_SIZE));
        }
