...
public class TaskGroupContainer extends AbstractContainer {
    ...
    public TaskGroupContainer(Configuration taskGroupConfig) {
        ...
        initCommunicator(taskGroupConfig);
        ...
        this.channelClazz = configuration.getString(CoreConstant.DATAX_CORE_TRANSPORT_CHANNEL_CLASS);
        this.taskCollectorClass = configuration.getString(CoreConstant.DATAX_CORE_STATISTICS_COLLECTOR_PLUGIN_TASKCLASS);
    }
    ...
    public void start() {
        try {
            ...
            long taskRetryIntervalInMsec = configuration.getLong(CoreConstant.DATAX_CORE_CONTAINER_TASK_FAILOVER_RETRYINTERVALINMSEC, 10000);

            long taskMaxWaitInMsec = configuration.getLong(CoreConstant.DATAX_CORE_CONTAINER_TASK_FAILOVER_MAXWAITINMSEC, 60000);

            List<Configuration> contentConfigList = configuration.getListConfiguration(CoreConstant.DATAX_JOB_CONTENT);

            if (LOG.isDebugEnabled()) {
                LOG.debug("taskGroup[{}]'s task configs[{}]", taskGroupId, JSON.toJSONString(contentConfigList));
            }

            int taskCountInThisTaskGroup = contentConfigList.size();

            LOG.info(String.format("taskGroupId=[%d] start [%d] channels for [%d] tasks.",
                    taskGroupId, channelNumber, taskCountInThisTaskGroup));

            containerCommunicator.registerCommunication(contentConfigList);

            Map<Integer, Configuration> taskConfigMap = buildTaskConfigMap(contentConfigList); //taskId与task配置
            List<Configuration> taskQueue = buildRemainTasks(contentConfigList); //待运行task列表
            Map<Integer, TaskExecutor> taskFailedExecutorMap = new HashMap<>(); //taskId与上次失败实例
            List<TaskExecutor> runningTaskExecutorList = new ArrayList<>(channelNumber); //正在运行task
            ...
            while (true) {
                ...
                if (taskQueue.isEmpty() && isAllTaskDone(runningTaskExecutorList) && containerCommunicator.collectState() == State.SUCCEEDED) {
    ...
    class TaskExecutor {
        ...
        public TaskExecutor(Configuration taskConf, int attemptCount) {
            ...
            // 通过设置thread的contextClassLoader，即可实现同步和主程序不通的加载器
            readerThread.setContextClassLoader(LoadUtil.getJarLoader(
        ...
        private AbstractRunner generateRunner(PluginType pluginType, List<TransformerExecution> transformerInfoExecs) {
            ...
            switch (pluginType) {
                case READER:
                    abstractRunner = LoadUtil.loadPluginRunner(pluginType,
                            this.taskConfig.getString(CoreConstant.JOB_READER_NAME));
                    abstractRunner.setJobConf(this.taskConfig.getConfiguration(
                    ...
                    if (transformerInfoExecs != null && transformerInfoExecs.size() > 0) {
                        recordSender = new BufferedRecordTransformerExchanger(
                                taskGroupId, taskId, channel, taskCommunication, pluginCollector, transformerInfoExecs);
                    } else {
                     ...
                     */
                    abstractRunner.setTaskPluginCollector(pluginCollector);
                    break;
                case WRITER:
                    abstractRunner = LoadUtil.loadPluginRunner(pluginType,
                            this.taskConfig.getString(CoreConstant.JOB_WRITER_NAME));
                    abstractRunner.setJobConf(this.taskConfig
                    ...
                    pluginCollector = ClassUtil.instantiate(
                            ...
                            configuration, this.taskCommunication,
                            PluginType.WRITER);
                    ((WriterRunner) abstractRunner).setRecordReceiver(new BufferedRecordExchanger(this.channel, pluginCollector));
                    /**
                     * 设置taskPlugin的collector，用来处理脏数据和job/task通信
                     */
                    abstractRunner.setTaskPluginCollector(pluginCollector);
            ...
            abstractRunner.setRunnerCommunication(this.taskCommunication);
