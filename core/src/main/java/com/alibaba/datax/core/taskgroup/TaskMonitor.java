...
public class TaskMonitor {
    ...
    @Getter
    private static class TaskCommunication {
        ...
        private TaskCommunication(Integer taskId, Communication communication) {
            ...
            lastUpdateCommunicationTs = System.currentTimeMillis();
        }

        private void report(Communication communication) {
            long now = System.currentTimeMillis();

            // 读取的数量增长，则变更当前记录, 优先判断这个条件，因为目的是不卡住，而不是expired
            if (CommunicationTool.getTotalReadRecordCount(communication) > lastAllReadRecords) {
                lastAllReadRecords = CommunicationTool.getTotalReadRecordCount(communication);
                lastUpdateCommunicationTs = now;
            } else if (isExpired(lastUpdateCommunicationTs)) {
                communication.setState(State.FAILED);
                communication.setTimestamp(now);
                communication.setThrowable(DataXException.build(CommonErrorCode.TASK_HUNG_EXPIRED,
                        String.format("task(%s) hung expired [allReadRecord(%s), elapsed(%s)]", taskId, lastAllReadRecords, (now - lastUpdateCommunicationTs))));
            }
        }
