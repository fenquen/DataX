...
public class FtpReader extends Reader {
    public static class Job extends Reader.Job {
        ...
        public void init() {
            ...
            if ("sftp".equals(protocol)) {
                //sftp协议
                port = originConfig.getInt(Key.PORT, Constant.DEFAULT_SFTP_PORT);
                this.ftpHelper = new SftpHelper();
            } else if ("ftp".equals(protocol)) {
                // ftp 协议
                port = originConfig.getInt(Key.PORT, Constant.DEFAULT_FTP_PORT);
                ftpHelper = new StandardFtpHelper();
            }

            ftpHelper.login(host, username, password, port, timeout, connectMode);
        }

        private void validateParameter() {
            //todo 常量
            protocol = originConfig.getNecessaryValue(Key.PROTOCOL, FtpReaderErrorCode.REQUIRED_VALUE);
            ...
            connectMode = originConfig.getUnnecessaryValue(Key.CONNECTPATTERN, Constant.DEFAULT_FTP_CONNECT_MODE, null);
            boolean connectPatternTag = "PORT".equals(connectMode) || "PASV".equals(connectMode);
            if (!connectPatternTag) {
                throw DataXException.build(FtpReaderErrorCode.ILLEGAL_VALUE,
                        String.format("不支持您配置的ftp传输模式: [%s]", connectMode));
            } else {
                this.originConfig.set(Key.CONNECTPATTERN, connectMode);
            }

            //path check
            String pathInString = originConfig.getNecessaryValue(Key.PATH, FtpReaderErrorCode.REQUIRED_VALUE);
            if (!pathInString.startsWith("[") && !pathInString.endsWith("]")) {
                path = new ArrayList<>();
                path.add(pathInString);
            } else {
                path = originConfig.getList(Key.PATH, String.class);
        ...
        public void prepare() {
            ...
            sourceFiles = ftpHelper.getAllFiles(path, 0, maxTraversalLevel);
    ...
    public static class Task extends Reader.Task {
        ...
        public void init() {//连接重试
            /* for ftp connection */
            readerSliceConfig = this.getPluginJobReaderWriterParamConf();
            host = readerSliceConfig.getString(Key.HOST);
            protocol = readerSliceConfig.getString(Key.PROTOCOL);
            username = readerSliceConfig.getString(Key.USERNAME);
            password = readerSliceConfig.getString(Key.PASSWORD);
            timeout = readerSliceConfig.getInt(Key.TIMEOUT, Constant.DEFAULT_TIMEOUT);

            this.sourceFiles = this.readerSliceConfig.getList(Constant.SOURCE_FILES, String.class);

            // sftp
            if ("sftp".equals(protocol)) {
                port = readerSliceConfig.getInt(Key.PORT, Constant.DEFAULT_SFTP_PORT);
                ftpHelper = new SftpHelper();
            } else if ("ftp".equals(protocol)) { // ftp 协议
                port = readerSliceConfig.getInt(Key.PORT, Constant.DEFAULT_FTP_PORT);
                connectPattern = readerSliceConfig.getString(Key.CONNECTPATTERN, Constant.DEFAULT_FTP_CONNECT_MODE);// 默认为被动模式
                ftpHelper = new StandardFtpHelper();
            }

            ftpHelper.login(host, username, password, port, timeout, connectPattern);
        }
        ...
        public void destroy() {
            try {
                ...
                String message = String.format("关闭与ftp服务器连接失败:host=%s, username=%s, port=%s", host, username, port);
                LOG.error(message, e);
            }
        }
        ...
        public void startRead(RecordSender recordSender) {
            ...
            for (String fileName : sourceFiles) {
                ...
                UnstructuredStorageReaderUtil.readFromStream(inputStream, fileName, readerSliceConfig, recordSender, getTaskPluginCollector());
                recordSender.flush();
            }
