...
public class SingleTableSplitUtil {
    ...
    public static List<Configuration> splitSingleTable(Configuration configuration, int adviceNum) {
        ...
        if (dataBaseType == DataBaseType.Oracle) {
            ...
            if (isStringType) {
                rangeList = RdbmsRangeSplitWrap.splitAndWrap(
                        ...
                        String.valueOf(minMaxPK.getRight()),
                        adviceNum,
                        splitPkName,
                        "'",
                        dataBaseType);
        ...
        // 只是用来打印的时候用
        List<String> querySplitSqlList = new ArrayList<>();

        if (CollectionUtils.isNotEmpty(rangeList)) {
            for (String range : rangeList) {
                Configuration tempConfig = configuration.clone();

                String querySplitSql = buildQuerySql(column, table, where) + (hasWhere ? " and " : " where ") + range;
                querySplitSqlList.add(querySplitSql);

                tempConfig.set(Key.QUERY_SQL, querySplitSql);
            ...
            Configuration tempConfig = configuration.clone();
            String tempQuerySql = buildQuerySql(column, table, where)
                    + (hasWhere ? " and " : " where ")
                    + String.format(" %s IS NOT NULL", splitPkName);
            querySplitSqlList.add(tempQuerySql);
        ...
        LOG.info("After split(), allQuerySql=[\n{}\n].", StringUtils.join(querySplitSqlList, "\n"));
    ...
    private static Pair<Object, Object> getPkRange(Configuration configuration) {
        ...
        Pair<Object, Object> minMaxPK = checkSplitPk(connection, pkRangeSQL, fetchSize, table, username, configuration);
     ...
     */
    private static Pair<Object, Object> checkSplitPk(Connection conn,
                                                     String pkRangeSQL, // select min(字段名字),max(字段名字) from table
                                                     int fetchSize,
                                                     String table,
                                                     String username,
        ...
        try {
            ...
            throw DataXException.build(DBUtilErrorCode.ILLEGAL_SPLIT_PK, e);
        } finally {
    ...
    private static boolean isLongType(int type) {
        boolean isLongType = type ==
                Types.BIGINT ||
                type == Types.INTEGER ||
                type == Types.SMALLINT ||
                type == Types.TINYINT;

        switch (SingleTableSplitUtil.dataBaseType) {
            case Oracle:
            case OceanBase:
                isLongType |= type == Types.NUMERIC;
                break;
            default:
                break;
        }
    ...
    /**
     * select min(字段名字),max(字段名字) from table
     */
    ...
    public static String genPKSql(String splitPK, String table, String where) {
        String template = "SELECT MIN(%s),MAX(%s) FROM %s";
        String pkRangeSQL = String.format(template, splitPK, splitPK, table);
    ...
    public static List<String> genSplitSqlForOracle(String splitPK,
        ...
        try {
            ...
            throw DataXException.build(
                    DBUtilErrorCode.ILLEGAL_SPLIT_PK, "DataX尝试切分表发生错误. 请检查您的配置并作出修改.", e);
        } finally {
            DBUtil.closeDBResources(rs, null, null);
        }
        ...
        if (splitedRangeSize >= 2) {
            ...
            if (isLongType(splitedRange.get(0).getRight())) {
                ...
                throw DataXException
                        .build(DBUtilErrorCode.ILLEGAL_SPLIT_PK,
