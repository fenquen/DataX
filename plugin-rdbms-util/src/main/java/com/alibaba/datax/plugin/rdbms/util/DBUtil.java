...
public final class DBUtil {
    ...
    public static String chooseJdbcUrl(final DataBaseType dataBaseType,
        ...
        try {
            return RetryUtil.executeWithRetry(() -> {
                ...
                throw new Exception("DataX无法连接对应的数据库，可能原因是：1) 配置的ip/port/database/jdbc错误，无法连接。2) 配置的username/password错误，鉴权失败。。");
            }, 7, 1000L, true);
            //warn: 7 means 2 minutes
        } catch (Exception e) {
            throw DataXException.build(DBUtilErrorCode.CONN_DB_ERROR,
                    String.format("数据库连接失败.:%s 找不到可连接的jdbcUrl", StringUtils.join(jdbcUrls, ",")), e);
        }
    }
    ...
    public static String chooseJdbcUrlWithoutRetry(DataBaseType dataBaseType,
                                                   ...
                                                   String password,
                                                   List<String> preSql,
                                                   boolean checkSlave) throws DataXException {

        if (null == jdbcUrls || jdbcUrls.isEmpty()) {
            throw DataXException.build(DBUtilErrorCode.CONF_ERROR,
                    String.format("您的jdbcUrl的配置信息有错, 因为jdbcUrl[%s]不能为空.", StringUtils.join(jdbcUrls, ",")));
        }
        ...
        throw DataXException.build(DBUtilErrorCode.CONN_DB_ERROR,
                String.format("数据库连接失败. 因为根据您配置的连接信息,无法从:%s 中找到可连接的jdbcUrl", StringUtils.join(jdbcUrls, ",")));
    }
